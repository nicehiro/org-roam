...
#+OPTIONS: toc:nil
#+roam_alias:
#+roam_tags: machine-learning linear-classification
...
，其中优化时最常用的方法是[[file:20201103101531-gradient_descent.org][Gradient Descent]]。
...
** 多类支持向量机损失
我们先介绍一种常见的计算损失的函数：[[file:20201103080641-多类支持向量机损失_svm.org][多类支持向量机损失（SVM）]]。
...
最后需要说明的是，函数项 $\max(0,-)$ 称为合页损失函数（[[file:20201103080921-hinge_loss.org][Hinge Loss]]）。
...
我们可以添加[[file:20201103081045-regularization.org][正则化]]惩罚项 $R(W)$ 来约束参数。
...
* Softmax 分类器
SVM 分类器把函数 $f(x_i,W)$ 的输出看作每个标签类别的分值，[[file:20201103080754-softmax.org][Softmax]] 分类器对输出的解释更直观（归一化的概率）。
...
*信息论* 真实分布 $p$ 和估计分布 $q$ 之间的[[file:20201117092705-cross_entropy.org][交叉熵]]为：
$$
