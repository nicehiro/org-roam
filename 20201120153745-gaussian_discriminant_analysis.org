...
它拟合的对象为 $p(x|y)$ ，并假设 $p(x|y)$ 服从[[file:20201118110534-multivariate_gaussian_distributiuon.org][多维正态分布]]。
...
使用[[file:20201103085205-maximum_likelihood_estimation.org][极大似然法]]求解，计算其 log-likelihood
...
这其实就是 [[file:20201103094628-logistic_regression.org][Logistic Regression]] $p(y=1|x)$ 的表达式。
