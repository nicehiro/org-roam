...
#+begin_export latex
\begin{algorithm}[H]
\caption{Flow Matching Training (CondOT Path)}
\label{alg:flow-matching}
\KwIn{Dataset $\mathcal{D} \sim p_{\text{data}}$, vector field network $u_t^{\theta}$}
\For{each training iteration}{
    Sample data $z \sim \mathcal{D}$\;
    Sample time $t \sim \text{Unif}[0,1]$\;
    Sample noise $\epsilon \sim \mathcal{N}(0, I_d)$\;
    $x \gets t z + (1-t)\epsilon$\;
    $\mathcal{L}_{\theta} \gets \| u_t^{\theta}(x) - (z - \epsilon) \|^2$\;
    Update $\theta$ via gradient descent on $\mathcal{L}_{\theta}$\;
}
\end{algorithm}
#+end_export
