...
\begin{align*}
\mathcal{L}
&= \bigl\|x_{t-1} - \theta(x_t)\bigr\|_2^2 \\
&= \Bigl\|\tfrac{1}{\alpha_t}\bigl(x_t - \beta_t \epsilon_t\bigr) - \theta(x_t)\Bigr\|_2^2
\qquad\text{(since }x_t=\alpha_t x_{t-1}+\beta_t\epsilon_t\text{)} \\
&= \Bigl\|\tfrac{1}{\alpha_t}\bigl(x_t - \beta_t \epsilon_t\bigr)
      - \tfrac{1}{\alpha_t}\bigl(x_t - \beta_t \,\epsilon_{\theta}(x_t,t)\bigr)\Bigr\|_2^2 \\
&= \tfrac{\beta_t^2}{\alpha_t^2}\,\bigl\|\epsilon_t - \epsilon_{\theta}(x_t,t)\bigr\|_2^2 \\
&\propto \bigl\|\epsilon_t - \epsilon_{\theta}(x_t,t)\bigr\|_2^2
\qquad\text{(drop the $t$-dependent factor; constant w.r.t.\ }\theta\text{)} \\
&= \bigl\|\epsilon_t - \epsilon_{\theta}(\alpha_t x_{t-1}+\beta_t \epsilon_t,\,t)\bigr\|_2^2 \\
&= \bigl\|\epsilon_t - \epsilon_{\theta}(\bar{\alpha}_t x_0 + \bar{\beta}_t \bar{\epsilon}_t,\,t)\bigr\|_2^2,
\end{align*}
